...
export default async function StatusPage() {
  ...
  return (
    <div className="relative flex min-h-screen flex-col items-center justify-center bg-slate-950 px-4 py-12">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_120%,rgba(20,184,166,0.05),transparent_50%)]" />
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/60 backdrop-blur-xl p-8 shadow-2xl">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              <span className="text-xs text-slate-500 font-mono select-all">{apiUrl}</span>
            </div>
          </div>

          <div className="mt-8 space-y-6">
            {/* Overall status indicator */}
            <div className="flex items-center justify-between rounded-xl bg-slate-950/50 p-4 border border-slate-800">
              ...
              {isHealthy ? (
                <span className="inline-flex items-center rounded-full bg-emerald-500/10 px-3 py-1 text-xs font-semibold text-emerald-400 border border-emerald-500/20">
                  <ShieldCheck className="mr-1 h-3.5 w-3.5" /> Healthy
                </span>
              ) : (
                <span className="inline-flex items-center rounded-full bg-red-500/10 px-3 py-1 text-xs font-semibold text-red-400 border border-red-500/20">
                  <ShieldAlert className="mr-1 h-3.5 w-3.5" /> Unhealthy / Offline
                </span>
              )}
            </div>
              ...
              <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-4">
                ...
                <div className="mt-3">
                  {data ? (
                    ...
                    <span className="text-xs text-slate-500 font-medium">Unknown</span>
                  )}
                </div>
              </div>
              ...
              <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-4">
                ...
                <div className="mt-3">
                  {data ? (
                    <span
                      className={`text-xs font-medium ${
                        data.redis === 'connected' ? 'text-emerald-400' : 'text-red-400'
                      }`}
                    >
                      {data.redis}
                    </span>
            ...
            <div className="text-xs text-slate-500 bg-slate-950/30 p-3 rounded-lg border border-slate-850">
              <p className="font-semibold text-slate-400 mb-1">Testing Guide:</p>
              <ul className="list-disc pl-4 space-y-1">
                <li>Make sure the backend is running on <code className="text-teal-400">port 8000</code>.</li>
            ...
            <Link
              href="/status"
              className="flex items-center justify-center w-full rounded-xl border border-slate-800 bg-slate-950/80 hover:bg-slate-900 py-3 text-xs font-bold text-slate-300 transition-colors"
            >
