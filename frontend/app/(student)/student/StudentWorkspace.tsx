...
export default function StudentWorkspace({ initialTargets, selectedTargetId, mode = 'homework', userId }: StudentWorkspaceProps) {
  ...
  // Homework workflow & MP session states
  const [hwStatuses, setHwStatuses] = useState<Record<string, HomeworkStatus>>({});
  const [activeMpModalTarget, setActiveMpModalTarget] = useState<WeeklyTarget | null>(null);
  ...
  const handleStartHomework = async (target: WeeklyTarget) => {
    // If MP package is not completed yet (and mode is homework), prompt MP modal first!
    const st = hwStatuses[target.id];
    if (mode === 'homework' && st && st.mp_status !== 'completed') {
      setActiveMpModalTarget(target);
      return;
    }
